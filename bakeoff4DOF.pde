...
void draw() {
  ...
  rotate(logoRotation - (c_angle - currAngle));
  noStroke();
  fill(60, 60, 192, 192);
...
void mouseDragged(){
    ...
    //logoRotation =  q_angle + dangle; //Apply the rotation
    //if (logoRotation>=360) logoRotation -= 360; //clamping

    //currAngle = atan2(mouseY - logoY, mouseX - logoX);
...
public boolean checkForSuccess()
{
  ...
  println("Close Enough Rotation: " + closeRotation + " (rot dist="+calculateDifferenceBetweenAngles(d.rotation, degrees(logoRotation))+")");
